...
# We have fields for command, tool, action, commands, success, and error. This covers all \
# potential outcomes from each agent’s logic.
        ...
        try:
            ...
            if result.output.action == "route":
                if result.output.tool == "imagemagick":
                    ...
                    if img_result.output.command:
                        deps.commands.append(img_result.output.command)
                        console.print(f"[green]Added ImageMagick command:[/green] [dim]{img_result.output.command}[/dim]")
                        
                elif result.output.tool == "ffmpeg":
                    # Route to FFmpeg agent
                    ff_result = await ffmpeg_agent.run(instruction, deps=deps)
                    if ff_result.output.command:
                        deps.commands.append(ff_result.output.command)
                        console.print(f"[green]Added FFmpeg command:[/green] [dim]{ff_result.output.command}[/dim]")
            
            elif result.output.action == "list":
                display_commands(deps.commands)
                        
            elif result.output.action == "combine":
                if deps.commands:
                    ...
                    if combine_result.output.commands:
                        display_commands(combine_result.output.commands, "Combined commands:")
                            
                        # Execute combined commands
                        console.print("[yellow]Executing commands...[/yellow]")
                        exec_result = await executing_agent.run(
                            "Execute these commands:\n" + "\n".join(
                                f"{i}. {cmd}" for i, cmd in enumerate(combine_result.output.commands, 1)
                            ),
                            deps=deps
                        )
                        
                        if exec_result.output.success:
                            console.print("[green]All commands executed successfully[/green]")
                        else:
                            console.print(f"[red]Execution error: {exec_result.output.error}[/red]")
                        
                        # Clear commands once executed
                        deps.commands.clear()
                    else:
                        console.print("[red]Error: Failed to combine commands[/red]")
                        console.print(f"Combining agent response: {combine_result.output}")
                else:
...
#Out[]:
"""Error processing instruction: status_code: 400, model_name: gemma3:4b, body: {'message': 
'registry.ollama.ai/library/gemma3:4b does not support tools', 'type': 'api_error', 'param': None, 'code': 
None}"""